...
def test_atualizar_produto(client, prod_salvo):
    pid = prod_salvo["id"]
    res = client.put(f"/produtos/{pid}", json={"nome": "Notebook Pro", "preco": 4200.00})
    assert res.status_code == 200
    corpo = res.json()
    assert corpo["nome"] == "Notebook Pro"
    assert corpo["preco"] == 4200.00
    assert corpo["estoque"] == prod_salvo["estoque"]


def test_atualizar_campo_unico(client, prod_salvo):
    pid = prod_salvo["id"]
    res = client.put(f"/produtos/{pid}", json={"estoque": 99})
    assert res.status_code == 200
    assert res.json()["estoque"] == 99
    assert res.json()["nome"] == prod_salvo["nome"]


def test_atualizar_id_nao_existe(client):
    res = client.put("/produtos/99999", json={"nome": "Qualquer"})
    assert res.status_code == 404


def test_atualizar_preco_invalido(client, prod_salvo):
    pid = prod_salvo["id"]
    res = client.put(f"/produtos/{pid}", json={"preco": -10.0})
